...
observer_jar="$project_root/agent/build/experiments/background-annotation-observer-v1.jar"
...
"$javac_bin" --release 8 -d "$classes_dir" \
    ...
    "$experiment_dir/BackgroundAnnotationObserverAgentV1.java"
...
unzip -tq "$canary_jar"
"$jar_bin" cfm "$observer_jar" "$experiment_dir/observer-manifest.mf" \
    -C "$classes_dir" BackgroundAnnotationObserverAgentV1.class
unzip -tq "$observer_jar"
printf 'Built %s\n' "$output_jar"
printf 'Built %s\n' "$canary_jar"
printf 'Built %s\n' "$observer_jar"
